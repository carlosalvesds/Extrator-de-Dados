...
def extrair_dados_pdf(file):
    ...
    match_nota = re.search(r'NOTA\s*FISCAL.*?[N\u00baºoO]\s*(\d{6,})', full_text, re.IGNORECASE)
    if match_nota:
        dados["NumeroNota"] = match_nota.group(1)

    match_data = re.search(r'DATA\s*DE\s*EMISS[\u00c3A]O\s*[:\s]*(\d{2}/\d{2}/\d{4})', full_text, re.IGNORECASE)
    if match_data:
...
def gerar_planilha(dados_extraidos):
    ...
    for uc, dados in dados_extraidos.items():
        ...
        for col_num, valor in enumerate(valores, start=1):
            ...
            cell.alignment = Alignment(horizontal="center", vertical="center")
            if col_num == 5 and valor is not None:
